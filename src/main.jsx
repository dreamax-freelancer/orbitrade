...
function App() {
  ...
  return <main>
    ...
    <section className="hero shell" id="top" data-nav-section>
      ...
      <div className="hero-copy" data-reveal>
        ...
        <p className="hero-text">Orbit turns complex market structure into clear, risk-aware trade ideas—so every decision feels faster, calmer, and backed by data.</p>
         <div className="hero-buttons"><button className="button" onClick={() => enterAuth('signup')}><span>Analyze a chart</span><Icon name="arrow"/></button><button className="video-btn" onClick={() => document.querySelector('#product')?.scrollIntoView({ behavior: 'smooth' })}><span className="play"><Icon name="play" size={12}/></span>See Orbit in action</button></div>
    ...
    <section className="cta shell"><div className="cta-aurora"/><div><div className="eyebrow"><span className="live-dot"/> Your edge is waiting</div><h2>Trade smarter<br/><em>starting today.</em></h2></div><div><p>Join thousands of traders building a calmer, more consistent relationship with the market.</p><button className="button" onClick={() => enterAuth('signup')}><span>Get started free</span><Icon name="arrow"/></button></div></section>
